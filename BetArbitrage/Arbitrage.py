...
class Arbitrage:
    ...
    def Co(M,C,N):
        """
        Metodo donde se hace coincidir los partidos y sus respectivas cuotes
        """
    ...
    def Tennis():
        """
        Metodo donde se buscan los partidos de tennis y sus respectivas cuotas para cada casa de apuestas
        """
        ...
        n = len(name_w)
        m_tennis_wplay=[name_w[i-1].upper()+' '+name_w[i].upper() for i in range(1,n,2)]
        ...
        mt_rush = [i.text for i in bot4]

        bot5 = driver.find_elements(By.XPATH,'//div[@class="OutcomeButton__Odds-sc-1anyy32-5 gfVAbN"]')
        ct_rush = [i.text for i in bot5]

        time.sleep(2) 
        driver.quit()

        n = len(mt_rush)
        m_tennis_rush=[mt_rush[i-1].upper()+' '+mt_rush[i].upper() for i in range(1,n,2)]
      
        m_tennis_rush=list(map(lambda x:re.sub(",","",x),m_tennis_rush))
        
        n = len(ct_rush)
        c_tennis_rush=[[float(ct_rush[i-1]),float(ct_rush[i])] for i in range(1,n,2)]
        ...
        M_tennis = [m_tennis_bwin,m_tennis_rush,m_tennis_wplay,m_tennis_zamba] #Matches tennis
        C_tennis = [c_tennis_bwin,c_tennis_rush,c_tennis_wplay,c_tennis_zamba] #Cuotas Matches tennis
        M_tennis2 = ["bwin","rush","wplay","zamba"] #Nombre casas de apuestas
    ...
    def TableTennis():
        """
        Metodo donde se buscan los partidos de tennis de mesa y sus respectivas cuotas para cada casa de apuestas
        """ 
        ...
        n = len(name_w)
        m_tablet_wplay=[name_w[i-1].upper()+' '+name_w[i].upper() for i in range(1,n,2)]
        
        n = len(price_w)
        c_tablet_wplay=[[float(price_w[i-1]),float(price_w[i])] for i in range(1,n,2)]
        ...
        n = len(mt_rush)
        mt_rush2=[mt_rush[i-1].upper()+' '+mt_rush[i].upper() for i in range(1,n,2)]
            
        m_tablet_rush=list(map(lambda x:re.sub(",","",x),mt_rush2))
   
        n = len(ct_rush)
        c_tablet_rush=[[float(ct_rush[i-1]),float(ct_rush[i])] for i in range(1,n,2)]
        ...
        M_tablet = [m_tablet_bwin,m_tablet_rush,m_tablet_wplay,m_tablet_zamba] #Matches Tennis de mesa
        C_tablet = [c_tablet_bwin,c_tablet_rush,c_tablet_wplay,c_tablet_zamba] #Cuotas Matches Tennis de mesa
        N_tablet = ["BWIN","RUSH","WPLAY","ZAMBA"]  #Nombres Casas de Apuestas
        Arbitrage.Co(M_tablet,C_tablet,N_tablet)

    def Volley():
        """
        Metodo donde se buscan los partidos de Volley y sus respectivas cuotas para cada casa de apuestas
        """
        ...
        m_volley_bwin = [match.text for match in matches]
        c_bwin = [cuot.text for cuot in cuotes]
        ...
        n = len(name_w)
        m_volley_wplay=[name_w[i-1].upper()+' '+name_w[i].upper() for i in range(1,n,2)]
       
        n = len(price_w)
        c_volley_wplay=[[float(price_w[i-1]),float(price_w[i])] for i in range(1,n,2)]
        ...
        n = len(ct_rush)
        c_volley_rush=[[float(ct_rush[i-1]),float(ct_rush[i])] for i in range(1,n,2)]
        ...
        M_volley = [m_volley_bwin,m_volley_rush,m_volley_wplay,m_volley_zamba] #Matches volley
        C_volley = [c_volley_bwin,c_volley_rush,c_volley_wplay,c_volley_zamba] #Cuotas Matches volley
        N_volley = ["BWIN","RUSH","WPLAY","ZAMBA"]  #Nombres casas de apuestas
        Arbitrage.Co(M_volley,C_volley,N_volley)

    def Esports():
        """
        Metodo donde se buscan los partidos de Easports y sus respectivas cuotas para cada casa de apuestas
        """

        page_w  = requests.get("https://apuestas.wplay.co/es/s/ESPS/eSports")
        soup_w = BeautifulSoup(page_w.content, 'html.parser')
        soup_w = soup_w.prettify().split("Próximas Apuestas de eSports")
        soup_w=BeautifulSoup(soup_w[1],'html.parser')
        price_w=soup_w.select(r'span.price')
        name_w = soup_w.select(r'span.seln-name')
        name_w=[a.get_text() for a in name_w]
        ...
        n = len(name_w)
        m_esports_wplay=[name_w[i-1].upper()+' '+name_w[i].upper() for i in range(1,n,2)]
       
        n = len(price_w)
        c_esports_wplay=[[float(price_w[i-1]),float(price_w[i])] for i in range(1,n,2)]
        ...
        n = len(ct_rush)
        c_esports_rush=[[float(ct_rush[i-1]),float(ct_rush[i])] for i in range(1,n,2)]
        ...
        M_esports = [m_esports_rush,m_esports_zamba,m_esports_wplay]
        C_esports = [c_esports_rush,c_esports_zamba,c_esports_wplay]
        N_esports = ["RUSH","ZAMBA","WPLAY"]

        Arbitrage.Co(M_esports,C_esports,N_esports)  


    def Futbol():
        """
        Metodo donde se buscan los partidos de futbol y sus respectivas cuotas de mas y menos goles para cada casa de apuestas
        """
        ...
        print(m_futbol_bwin)
        print(mas_futbol_bwin)
        print(cmas_futbol_bwin)
        ...
        def Empar(M1,M2,C1,C2,CM1,CM2):
            OPORF = []
            EmpM = []
            for i in range(M1):
                for j in range(M2):
                    if M1[i][0:3] in M2[j][0:3]:
                        for k1 in range(C1[i]):
                            for k2 in range(C2[j]):
                                if C1[i][k1]==C2[j][k2]:
                                    OPORF.append([1/CM1[i][2*k1]+1/CM2[j][2*k2],1/CM1[i][2*k1+1]+1/CM2[j][2*k2+1]])
                                    EmpM.append(M1[i])

            return EmpM,OPORF
       
        Empar(m_futbol_bwin,m_futbol_zamba,cmas_futbol_bwin,cmas_futbol_zamba,mas_futbol_bwin,mas_futbol_zamba)
 

    def All():
        """
        Metodo para mostrar las oportunidades de arbitraje de todos los deportes 
        """
        Arbitrage.Tennis()
        Arbitrage.Volley()   
